#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Inferencer for ModelDOA (phase2): pred-DOA AF + separation."""
import math
...
from model_doa import angle_rad_to_bin, bin_to_angle_rad


class InferencerDOA(BaseInferencer):
    def __init__(self, config, checkpoint_path, output_dir):
        ...
        self.num_doa_bins = int(
            doa_cfg.get("num_doa_bins", model_args.get("num_doa_bins", 18))
        )
    ...
    def _load_model(model_config, checkpoint_path, device):
        ...
        if checkpoint_path.suffix == ".pth":
            ...
            # model_0050_best.pth or model_0050.pth
        ...
        print(f"Loading ModelDOA checkpoint (epoch == {epoch})...")
        ...
        model.to(device)
        model.eval()
        # Ensure phase2 behavior even if ckpt meta differs
        if hasattr(model, "training_phase"):
            model.training_phase = int(model_config["args"].get("training_phase", 2))
            model.use_pred_doa_for_af = True
            if hasattr(model, "_apply_training_phase"):
                model._apply_training_phase()
        return model, epoch
    ...
    def __call__(self):
        ...
        for i, data in tqdm(enumerate(self.dataloader), desc="Inference-DOA"):
            ...
            # ModelDOA returns (ests, doa_logits); GT doa is unused for AF in phase2
            ...
            # ---- DOA metrics (target speaker = index 0) ----
            ...
            # wrap to [0, pi] for half-plane reporting (optional)
