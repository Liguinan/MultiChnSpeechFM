...
class DFComputer(nn.Module):
    ...
    def init_mic_pos(self):
        ...
        self.mic_position = np.array(
            [[-0.0425, 0], [-0.02125, 0.03680], [0.02125, 0.03680], [0.0425, 0], [0.02125, -0.03680],
             [-0.02125, -0.03680], [0, 0], [0, 0]])
    ...
    def forward(self, x, directions, nspk, geometry):
        ...
        df = []
        if 'LPS' in self.input_feature:
            lps = torch.log(mag_avg ** 2 + self.epsilon)
            lps = self.ln_LPS(lps)
            # [B, F, T] -> [B, 1, F, T] 
            lps = lps.unsqueeze(dim=1) 
            # [B, 1, F, T] -> [B, len(geometry), F, T]
            lps = lps.expand(-1, len(geometry), -1, -1) 
            df.append(lps)
        ...
        if 'AF' in self.input_feature:
            ...
            # get steering vector 
            # [B, max_spk_num] -> [B, mic_pair, max_spk_num, F]
            stv_real, stv_imag = self.get_stv(directions)
            AF = self.get_AF(stv_real, stv_imag, ipd4AF_real, ipd4AF_imag, nspk)  # [B, F, T]
            AF = AF.unsqueeze(dim=1)  # [B, 1, F, T]
            AF = AF.expand(-1, len(geometry), -1, -1)
            df.append(AF)
        # import pdb; pdb.set_trace()
        # -> df: [B, mic_pair, 3*F=3*257=771, T]
        # -> mag: [B, all_mic+1=8, F, T]
        # -> phase: [B, all_mic+1=8, F, T]
        df = torch.cat(df, dim=2)
        return df, mag, phase
