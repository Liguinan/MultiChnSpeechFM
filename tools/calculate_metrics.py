#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2025/11/17 14:34
# @Author  : Guinan Li
# @File    : calculate_metrics.py.py
...
def pre_processing(est, ref, specific_dataset=None):
    ...
    if not specific_dataset:
        # In the default case, the two lists should be one-to-one
        check_two_aligned_list(reference_wav_paths, estimated_wav_paths)
    else:
        # For different datasets, manual alignment is performed to ensure that the two lists correspond to each other
        reordered_estimated_wav_paths = []
        if specific_dataset == "dns_1":
            # Use the suffix of the reference file name to find the corresponding estimated file name
            for ref_path in reference_wav_paths:
                for est_path in estimated_wav_paths:
                    est_basename = get_basename(est_path)
                    if "clean_" + "_".join(est_basename.split("_")[-2:]) == get_basename(
                            ref_path
                    ):
            ...
            # Reference_channel = 0
            # find the corresponding clean speech
            reference_channel = 0
...
def check_two_aligned_list(a, b):
    assert len(a) == len(b), f"The length of two lists are not equal: {len(a)} vs {len(b)}"
    for z, (i, j) in enumerate(zip(a, b), start=1):
        assert get_basename(i) == get_basename(j), (
            f"There are different names in {z}" f"\n\t {i}" f"\n\t{j}."
        )
...
def compute_metric(reference_wav_paths, estimated_wav_paths, sr, metric_type="SI_SDR"):
    ...
    assert metric_type in metrics_dict, f"Unsupported metric: {metric_type}."
...
def main(args):
    ...
    # Use the specified scp file or directory to get all wav samples
    ...
    for metric_type in metric_types.split(","):
        ...
        if export_dir:
            ...
            metric_seq = [
                [basename, metric_value] for basename, metric_value in metrics_result_store
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="[WIP] Set the limit of the number of samples.",
    )
    parser.add_argument(
        "--offset",
        type=int,
        default=0,
        help="[WIP] Set the offset of the number of samples.",
    )
    parser.add_argument(
        "-S",
        "--specific_dataset",
        type=str,
        default="",
        help="Specify dataset type, e.g. DNS_1, DNS_2",
    )
