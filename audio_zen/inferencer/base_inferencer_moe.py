...
def _dataset_split_name(dataset_path):
    """Map pkl path to train / val / test for output dirs."""
    parts = [p.lower() for p in Path(dataset_path).parts]
    for part in parts:
        if part.startswith("train"):
            return "train"
        if part.startswith("test"):
            return "test"
        if part.startswith("dev") or part.startswith("val"):
            return "val"
...
class BaseInferencer:
    def __init__(self, config, checkpoint_path, output_dir):
        ...
        dataset_name = _dataset_split_name(config["dataset"]["args"]["dataset_path"])
        ...
        # geometry
        self.geometry_all = config["array_geometry"]["training_array_geometries"] + config["array_geometry"]["test_array_geometries"]
        print(f"all geometry: {self.geometry_all}")
        self.geometry_idx = config["array_geometry"]["inference_geometry_idx"]
        print(f"geometry idx is: {self.geometry_idx}")
        self.geometry = self.geometry_all[self.geometry_idx]
        print(f"selected geometry is: {self.geometry}")

        sub_dir = f"array_{'_'.join([str(x) for x in self.geometry])}"
        self.enhanced_dir = root_dir / sub_dir / f"enhanced_{str(epoch).zfill(4)}_{dataset_name}"
        self.noisy_dir = root_dir / sub_dir / f"noisy_{dataset_name}"
        self.clean_dir = root_dir / sub_dir / f"reference_{dataset_name}"
        self.log_dir = root_dir / sub_dir / f"log_{str(epoch).zfill(4)}_{dataset_name}"
    ...
    def __call__(self):
        ...
        for i, data in tqdm(enumerate(self.dataloader), desc="Inference"):
            ...
            # data["geometry_id"] = torch.tensor(self.geometry_idx, dtype=torch.long)
            ...
            ests, _,_ = self.model([data["mix"], data["src_doa"], data["spk_num"], data['ori_wav_len'], data["geometry"], data["geometry_id"]])
