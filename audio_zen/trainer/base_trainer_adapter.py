...
class BaseTrainer:
    ...
    def _preload_model(self, model_path):
        ...
        for k, v in model_static_dict.items():
            if k.startswith("fusion_block.0"):
                # import pdb; pdb.set_trace()
                updated_model_static_dict[k.replace('fusion_block.0', 'fusion_block_4.0')] = v
            elif k.startswith("fusion_block.1"):
                updated_model_static_dict[k.replace('fusion_block.1', 'fusion_block_5.0')] = v
            elif k.startswith("fusion_block.2"):
                updated_model_static_dict[k.replace('fusion_block.2', 'fusion_block_6.0')] = v
            else:
