...
class BaseTrainer:
    ...
    def _preload_model(self, model_path):
        ...
        model_path_2 = "/path/to/MultiChnSpeechFMExperiments/v4/TAC_Based_MultiChnNet_train_adapter_residual_from_pretrain_SAT_lr0.0005_adapter_position_0/fixed_array_0235/train_adapter_residual_from_pretrain_SAT_adapter_position_0_fixed_array_0235/checkpoints/best_model.tar"
        model_checkpoint_2 = torch.load(model_path_2, map_location="cpu")
        model_static_dict_2 = model_checkpoint_2["model"]
        ...
        for k, v in model_static_dict.items():
            if fnmatch.fnmatch(k,"expert_adapter.*list.0*"):
                # import pdb; pdb.set_trace()
                updated_model_static_dict[k] = model_static_dict_2[k]
                print(f"已替换: {k}")
            else:
                updated_model_static_dict[k] = v
        # import pdb; pdb.set_trace()
