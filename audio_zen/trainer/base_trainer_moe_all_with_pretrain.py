...
class BaseTrainer:
    def __init__(
        ...
        if self.rank == 0:
            ...
            prepare_empty_dir([self.checkpoints_dir, self.logs_dir, self.val_loss_dir, self.meta_files_dir], resume=resume)
    ...
    def _preload_model(self, model_path):
        ...
        if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
            self.model.module.load_state_dict(model_static_dict_adapter3, strict=False)
        else:
            self.model.load_state_dict(model_static_dict_adapter3, strict=False)
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            save_meta_file_path = f"{self.meta_files_dir}/epoch_{epoch}_loss.txt"
