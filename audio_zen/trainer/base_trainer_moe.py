...
class BaseTrainer:
    ...
    def _preload_model(self, model_path):
        ...
        # import pdb; pdb.set_trace()
        model_checkpoint_012345_adapter2 = "/path/to/MultiChnSpeechFMExperiments/v4/seen/TAC_Based_MultiChnNet_train_adapter_residual_from_pretrain_lr0.005_adapter_position_0_fixed_array_012345_all/train_adapter_residual_from_pretrain_position_0_fixed_array_012345/checkpoints/best_model.tar"
        model_checkpoint_0246_adapter1 = "/path/to/MultiChnSpeechFMExperiments/v4/seen/TAC_Based_MultiChnNet_train_adapter_residual_from_pretrain_lr0.005_adapter_position_0_fixed_array_0246_all/train_adapter_residual_from_pretrain_position_0_fixed_array_0246/checkpoints/best_model.tar"
        model_checkpoint_0235_adapter0 = "/path/to/MultiChnSpeechFMExperiments/v4/seen/TAC_Based_MultiChnNet_train_adapter_residual_from_pretrain_lr0.005_adapter_position_0_fixed_array_0235_all/train_adapter_residual_from_pretrain_position_0_fixed_array_0235/checkpoints/model_0006_best.pth"
        ...
        for k, v in self.model.module.state_dict().items():
            if "moe_weights" in k:
                continue
            # import pdb; pdb.set_trace()
            if fnmatch.fnmatch(k,"expert_adapter.*list.0*"):
                # import pdb; pdb.set_trace()
                updated_model_static_dict[k] = model_static_dict_adapter0[k]
                print(f"已替换: {k}")
            elif fnmatch.fnmatch(k,"expert_adapter.*list.1*"):
                # import pdb; pdb.set_trace()
                k_real = k.replace("1", "0")
                updated_model_static_dict[k] = model_static_dict_adapter1[k_real]
                print(f"已替换: {k}")
            elif fnmatch.fnmatch(k,"expert_adapter.*list.2*"):
                # import pdb; pdb.set_trace()
                k_real = k.replace("2", "0")
                updated_model_static_dict[k] = model_static_dict_adapter2[k_real]
                print(f"已替换: {k}")
            elif fnmatch.fnmatch(k,"expert_adapter.*list.3*"):
                # import pdb; pdb.set_trace()
                k_real = k.replace("3", "0")
        ...
        for name, param in self.model.module.named_parameters():
            if "moe_weights" not in name and 'expert_adapter' not in name:
                param.requires_grad = False
            else:
                print(f"unfrozen param: {name}")
            # if param.requires_grad == False:
            #     print(f"non_trainable_params param: {name}")
